...
class SimpleTree<T> {
    ...
    public ArrayList<T> EvenTrees() {
        ...
        if (this.Count() % 2 == 0) {
            addEdgeForDelete(getRoot(), listResult);
        }
        return listResult;
    }

    private void addEdgeForDelete(SimpleTreeNode<T> node, ArrayList<T> listDeleteEdge) {
        if (node == null) {
            return;
        }
        SimpleTree<T> treeNew = new SimpleTree<>(node);
        if (treeNew.Count() % 2 == 0 && node.getParent() != null) {
            listDeleteEdge.add(node.getParent().getNodeValue());
            listDeleteEdge.add(node.getNodeValue());
        }
        for (SimpleTreeNode<T> nodeChild : node.getChildren()) {
            addEdgeForDelete(nodeChild, listDeleteEdge);
        }
    }
}
